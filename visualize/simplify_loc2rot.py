...
import smplx
import h5py
from visualize.joints2smpl.src.smplify import SMPLify3D, RootOnly_SMPLify3D
...
class joints2smpl:
    ...
    def __init__(self, num_frames, device_id, cuda=True, load_smplx=False, body_models_path=None):
        ...
        self.fix_foot = False
        self.load_smplx = load_smplx

        if body_models_path is not None:
            config.SMPL_MODEL_DIR = os.path.join(body_models_path, config.SMPL_MODEL_DIR)
            config.GMM_MODEL_DIR = os.path.join(body_models_path, config.GMM_MODEL_DIR)
            config.SMPL_MEAN_FILE = os.path.join(body_models_path, config.SMPL_MEAN_FILE)
            config.Part_Seg_DIR = os.path.join(body_models_path, config.Part_Seg_DIR)
    ...
    def joint2smpl(self, input_joints, init_params=None):
        ...
        pred_pose = torch.zeros(self.batch_size, (self.n_thetas+1)*3).to(self.device)
        ...
        if init_params is None:
            ...
            pred_pose = self.init_mean_pose[:,:(self.n_thetas+1)*3]
        ...
        joint_diff = (new_opt_joints[:,:22,:].cpu().numpy() - input_joints)


        thetas = new_opt_pose.reshape(self.batch_size, self.n_thetas+1, 3)
        transl = new_opt_cam_t.clone().detach()
        ...
        thetas = torch.cat([thetas, transl], dim=1) # [1, 25, 6, 196]

        return thetas.clone().detach(), {'pose': new_opt_joints[:, :].clone().detach(), 
                                         'betas': new_opt_betas.clone().detach(), 'cam': new_opt_cam_t.clone().detach(),
                                         'vertices': new_opt_vertices.clone().detach(), 'joint_diff': joint_diff}
    
    def rootonly_joint2smpl(self, input_joints, init_params=None):
        self.smplify = RootOnly_SMPLify3D(smplxmodel=self.smplmodel,
                            batch_size=self.batch_size,
                            joints_category=self.joint_category,
                            num_iters=self.num_smplify_iters,
                            device=self.device)
        ...
        _smplify = self.smplify # if init_params is None else self.smplify_fast
        pred_root = torch.zeros(self.batch_size, 3).to(self.device)
        ...
        if init_params is None:
            ...
            pred_betas = init_params['betas'].to(self.device)
            pred_pose = init_params['pose'].to(self.device)
            pred_cam_t = self.cam_trans_zero
        ...
        return thetas.clone().detach(), {'pose': new_opt_joints[:, :24].flatten().clone().detach(), 
                                         'betas': new_opt_betas.clone().detach(), 'cam': new_opt_cam_t.clone().detach(),
                                         'vertices': new_opt_vertices.clone().detach(), 'joint_diff': joint_diff}
