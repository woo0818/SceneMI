...
class SMPLify3D():
    ...
    def __init__(self,
        ...
        self.pose_prior = MaxMixturePrior(prior_folder=config.GMM_MODEL_DIR,
                                          num_gaussians=8,load_smplx=self.load_smplx,
...
class RootOnly_SMPLify3D():
    ...
    # ---- get the man function here ------
    def __call__(self, init_root, init_pose, init_betas, init_cam_t, j3d, conf_3d=1.0, seq_ind=0):
        """Perform body fitting.
        Input:
            init_pose: SMPL pose estimate
            init_betas: SMPL betas estimate
        ...
        global_orient = init_root[:, :].detach().clone()
        betas = init_betas.detach().clone()
